...
## Operational scope — honest statement (Phase 4)
...
**Two things this does not close, stated plainly rather than implied away:**
...
- **DNS rebinding is not closed.** Between `url_safety`'s resolution and
  ...
  preserved in `Host` and SNI, which is not built.
