...
- **`groundkit.evals.beir.adapt_beir_dataset` supports binary-relevance BEIR collections only; a graded one is refused, not silently flattened.** `_load_qrels` raises `EvalError` on any qrels score outside `{0, 1}` — TREC-COVID and FiQA grade relevance 0–2 — because a `GoldSpan` is `(doc, quote)` with no field to carry a grade, so collapsing a 2 and a 1 into the same set membership would report *binary* nDCG that reads as directly comparable to a collection's published *graded* number. SciFact and NFCorpus's binary splits adapt cleanly (SciFact's own qrels are all 0/1, which is why `evals/beir/RESULTS-scifact-2026-08-21.md`'s comparison to BEIR's published number is valid); a graded collection needs the harness to grow a relevance grade first, which does not exist.
- **The BEIR adapter refuses Windows-reserved and case-colliding ids on every platform, not only where the collision is real.** `_validate_identifier` (`src/groundkit/evals/beir.py`) rejects a document or query id whose stem is `CON`/`PRN`/`AUX`/`NUL`/`COM1`–`9`/`LPT1`–`9` regardless of host OS, and `_reject_case_colliding_document_ids` rejects two ids differing only in case the same way everywhere. Both hazards are genuinely platform-dependent — the device-name reservation is Windows-only and itself build-dependent (measured on Windows 11 build 26340: `CON.txt`, `COM1` and `LPT1` round-trip fine, but a bare `NUL` silently accepts the write and reads back empty, discarding the document; POSIX gives none of these names any special meaning), and the case collision only happens on Windows and default macOS, never on Linux — but the adapter refuses both everywhere. That trades a legitimate id being rejected on a platform where nothing would actually go wrong for a corpus that adapts identically no matter which machine ran it: a benchmark corpus whose contents silently depended on the host OS would be the worse defect.
